...
The dashboard panels include: Session Burn Rate, Cost by Model, Tokens by Model, Token Breakdown, Prompt-Cache TTL Split, Per-Session Context Growth, Response Sizes, Tool Usage Ratio, Reply Latency, Tool Error Rate, Activity Heatmap, Cost by Context Size, Tokens by Context Size, Cost/Tokens by Agent Type, and Lines Added/Deleted (per-call churn from tool call arguments, stored on `tool_uses`) — Edit/Write plus the Bash shapes whose line counts are readable straight off the command text.
