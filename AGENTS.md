...
- **Aggregates are precomputed at ingest** into `usage_rollup` (grain: session × hour × model × is_main), `tool_rollup` (hour × project × model × tool), `tool_error_rollup` (hour × project × model × tool × error_kind), `dispatch_rollup` (hour × project × agent_type × agent_model) and `latency_rollup` — see SV-ROLLUP. The first two hold pure sums/counts/min/max and are summed up to the display bucket; they are valid only for buckets ≥ 1h, so the 24h view takes a live path.
