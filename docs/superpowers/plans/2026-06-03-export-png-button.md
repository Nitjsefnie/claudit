...
- **Deploy:** the deploy sync to `/opt/ccudash` must include `scripts/plots/` so the script is present at runtime. `--db-url` is passed explicitly, so the script's `/root/session-viz/.env` default is never used in production.
